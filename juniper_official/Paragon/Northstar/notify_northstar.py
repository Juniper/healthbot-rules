...
def generate_node_maitenance(mnt_time, hb_alert, **kwargs):
    ...
    if node_params:
        ...
        add_time = current_time + datetime.timedelta(days = int(mnt_time.split(":")[0]), hours = int(mnt_time.split(":")[1]), minutes = int(mnt_time.split(":")[2]))
        end_time = add_time.strftime("%Y-%m-%dT%H:%M:%S.000Z")
        simulation_payload='{"topoObjectType":"maintenance","topologyIndex":1,"elements":[{"topoObjectType":"node","index":'+str(node_index)+',"id":"'+str(node_id)+'"}],"user":"spadmin","name":"'+str(hb_alert)+'","comment":"'+str(hb_alert)+'","autocomplete": true,"startTime":"'+str(start_time)+'","endTime":"'+str(end_time)+'"}'
        try: 
